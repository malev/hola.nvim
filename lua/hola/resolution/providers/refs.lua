...
function RefsProvider:_load_refs()
  ...
  if self._refs_loaded and
     self._refs_file_path == refs_path and
     self._refs_file_mtime == current_mtime then
...
function RefsProvider:resolve(identifier)
  ...
  if value then
    ...
    if not value:match("{{.+:.+}}") then
      return nil, "Invalid reference target '" .. value .. "' - must contain at least one provider reference like {{provider:identifier}}"
    end
