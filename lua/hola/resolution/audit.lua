...
function AuditTrail:log_resolution_step(variable, provider_name, result)
	...
	-- Update total time
	self.variables[variable].total_resolution_time_ms = self.variables[variable].total_resolution_time_ms
...
function AuditTrail:get_variable_debug_output(variable)
	...
	for i, step in ipairs(var_info.resolution_chain) do
		...
		if step.output_info and step.output_info.starts_with then
			output_desc = string.format(' → "%s" (%d chars)', step.output_info.starts_with, step.output_info.length)
		...
		table.insert(lines, string.format("%s %s provider%s%s", prefix, step.provider, output_desc, timing))
	end
...
function AuditTrail:get_debug_summary(request_info)
	...
	if request_info and request_info.line and request_info.method and request_info.url then
		table.insert(
			lines,
			string.format("Request at line %d: %s %s", request_info.line, request_info.method, request_info.url)
		)
	else
