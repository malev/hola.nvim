...
function OAuthProvider:_load_toml()
  ...
  if self._toml_loaded and
     self._toml_file_path == toml_path and
     self._toml_file_mtime == current_mtime then
...
function OAuthProvider:resolve(identifier)
  ...
  if not service_config.token_url or not service_config.client_id or not service_config.client_secret then
    return nil, "OAuth service '" .. service_name .. "' missing required configuration (token_url, client_id, client_secret)"
  end
